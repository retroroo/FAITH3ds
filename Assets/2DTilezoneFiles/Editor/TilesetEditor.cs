...
public class TilesetEditor : EditorWindow {
	...
	#if UNITY_5_5_OR_NEWER
						UnityEditor.EditorUtility.SetSelectedRenderState( scriptableLayer.scriptableLayers[sl].layers[l].meshRenderer, UnityEditor.EditorSelectedRenderState.Hidden );
	#else
	...
	#if UNITY_5_5_OR_NEWER
						UnityEditor.EditorUtility.SetSelectedRenderState( networkScriptableLayer.scriptableLayers[sl].layers[l].meshRenderer, UnityEditor.EditorSelectedRenderState.Hidden );
	#else
	...
	void DrawSideControls() {
		...
		if (toolSelected == TileTool.Collisions) {
			...
			if( layerTileInfo != null ) {
				...
				if( EditorGUI.EndChangeCheck() ) {
					if( scriptableLayer != null && scriptableLayer.mainLayer != null ) {
						foreach (var item in scriptableLayer.scriptableLayers) {
							foreach (var layer in item.layers) {
								if( layer != null )
									layer.borderPixelOffset = scriptableLayer.mainLayer.borderPixelOffset;
							}
						}
					}
					#if UNITY_5_3 || UNITY_5_3_OR_NEWER
					if( networkScriptableLayer != null && networkScriptableLayer.mainLayer != null ) {
						foreach (var item in networkScriptableLayer.scriptableLayers) {
							foreach (var layer in item.layers) {
								if( layer != null )
									layer.borderPixelOffset = networkScriptableLayer.mainLayer.borderPixelOffset;
							}
						}
					}
					#endif
				...
				if( EditorGUI.EndChangeCheck() ) {
					...
					if( scriptableLayer != null && scriptableLayer.mainLayer != null ) {
						foreach (var item in scriptableLayer.scriptableLayers) {
							foreach (var layer in item.layers) {
								if( layer != null )
									layer.zoomFactor = scriptableLayer.mainLayer.zoomFactor;
							}
						}
					}
					#if UNITY_5_3 || UNITY_5_3_OR_NEWER
					if( networkScriptableLayer != null && networkScriptableLayer.mainLayer != null ) {
						foreach (var item in networkScriptableLayer.scriptableLayers) {
							foreach (var layer in item.layers) {
								if( layer != null )
									layer.zoomFactor = networkScriptableLayer.mainLayer.zoomFactor;
							}
						}
					}
					#endif
...
public class CreateTileLayer : EditorWindow {
	...
	bool createScriptableTileLayer = true;
